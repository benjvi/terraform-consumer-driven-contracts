...
usage() {
  ...
  cat >&2 <<- EOT
	...
	Usage: $0 <-o TF OUTPUTS> 
        ...
        Creates a JSON schema mandating the presence of the specified Terraform outputs in the metadata returned from the Concourse Terraform resource
...
SCHEMA_PROPERTIES_FIELD="$(echo $CONSUMED_OUTPUTS |  jq '{ properties: (map({ (.) : { "$ref": "#/definitions/tf_concourse_resource" } }) | add)}')"
SCHEMA_STUB=$(cat <<SETVAR
{
  "definitions": {
    "tf_concourse_resource": {
      "type": [ "string" , "array" ]
    }
  },
